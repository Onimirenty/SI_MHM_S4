...
(
    id serial PRIMARY KEY,
    idCompte INTEGER,
    Solde DECIMAL(100,2) not null,

    FOREIGN KEY (idCompte) REFERENCES compte(id)
);

CREATE TYPE natureTransaction AS ENUM ('debit','credit');
CREATE TYPE positionCompte AS ENUM ('actif','passif','charge','vente');


create table transactions
(
    id serial PRIMARY KEY,
    idCompte1 Integer not null,
    idCompte2 Integer not null,
    operation varchar(210) default('compte1 vers compte2'),
    valeur DECIMAL(100,2) not null,
    natureTransaction naturetransaction NOT NULL,
    position positioncompte NOT NULL,
    dates date NOT NULL,
...
INSERT into transactions (idCompte1, idCompte2, operation, valeur, natureTransaction, position, dates) VALUES
(5,6,'compte1 vers compte2',100,'debit','achat','2022-01-01');
...
create view solde as 
    select 
        compte.idcomptegeneral ,
        intitule_sous_compte,
        sous_compte,
        solde,
        natureTransaction,
        valeur
